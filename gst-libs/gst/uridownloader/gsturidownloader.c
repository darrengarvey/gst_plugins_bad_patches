    ...
    const gchar * uri, gint64 range_start, gint64 range_end)
{
   ...
   */
  GST_DEBUG_OBJECT (downloader, "Waiting to fetch the URI %s %p", uri, downloader);
  ...
  GST_DEBUG_OBJECT (downloader, "Wait is over %p", downloader);
  if (downloader->priv->cancelled) {
    if (downloader->priv->download) {
      g_object_unref (downloader->priv->download);
      downloader->priv->download = NULL;
...
quit:
  ...
  if(!download && downloader->priv->urisrc){
      downloader->priv->urisrc = NULL;
      GST_OBJECT_UNLOCK (downloader);
  ...
  if (download == NULL) {
      if (!downloader->priv->err) {
          /* err = g_new_error (GST_RESOURCE_ERROR, GST_RESOURCE_ERROR_OPEN_READ,
